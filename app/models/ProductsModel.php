<?php
    class ProductsModel extends Model {
        ...
        public function getTopPurchasedFishes() {
            $stmt = $this->db->prepare("SELECT * FROM products WHERE product_type = 'Fish' AND deleted != ? ORDER BY purchases DESC LIMIT 4");
            $stmt->execute([1]);
            return $stmt->fetchAll(PDO::FETCH_OBJ);
        }

        public function getproducts ($produc_type) {
            $stmt = $this->db->prepare("SELECT * FROM products WHERE product_type = '$produc_type' AND deleted != ?");
            $stmt->execute([1]);
            return $stmt->fetchAll(PDO::FETCH_OBJ);
        }

        public function getHighestPrice($produc_type) {
            $stmt = $this->db->prepare("SELECT product_price FROM products WHERE product_type = ? AND deleted != ? ORDER BY product_price DESC LIMIT 1");
            $stmt->execute([$produc_type, 1]);
            return $stmt->fetch(PDO::FETCH_OBJ);
        }
        public function getCategories($produc_type) {
            $stmt = $this->db->prepare("SELECT DISTINCT product_category FROM products WHERE product_type = ? AND deleted != ?");
            $stmt->execute([$produc_type, 1]);
            return $stmt->fetchAll(PDO::FETCH_OBJ);
        }
        ...
        public function getProductById($id) {
            $stmt = $this->db->prepare("SELECT * FROM products WHERE product_id = ? AND deleted != ?");
            $stmt->execute([$id, 1]);
            return $stmt->fetch(PDO::FETCH_OBJ);
        }     
        ...
        public function filterProductsByCategory($categories) {
            ...
            $stmt = $this->db->prepare("SELECT * FROM products WHERE product_category IN ($placeholders) AND deleted != ?");
        ...
        public function getAllProduct() {
            $stmt = $this->db->prepare("SELECT * FROM products WHERE deleted != ?");
            $stmt->execute([1]);
            return $stmt->fetchAll(PDO::FETCH_OBJ);
        }
        public function removeProduct($product_id){
            $stmt =$this->db->prepare("UPDATE products SET deleted = ? WHERE product_id = ? ");
